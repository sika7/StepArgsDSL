...
| 要素         | 正規表現                                           | 説明                                      |
| ------------ | -------------------------------------------------- | ----------------------------------------- |
| ステップ宣言 | `^---\s*([\p{L}\p{N}_\-\s]+?)\s*---$`              | 段階的Unicode対応（文字・数字・記号）     |
| 複数行開始   | `^([\p{L}\p{N}_\-]+)\[([\p{L}\p{N}_]+):<<<(.*)$`   | 段階的Unicode対応、`<<<`の後は空白のみ許可 |
| 単行引数     | `^([\p{L}\p{N}_\-]+)\[([\p{L}\p{N}_]+):((?:[^<\]]|\\.)*?)\]$` | 段階的Unicode対応、エスケープ処理対応     |
...
このパーサーは**段階的Unicode対応**を採用しています：

- **\p{L}**: Unicode文字プロパティで「文字」を表す（ひらがな、カタカナ、漢字、アルファベットなど）
- **\p{N}**: Unicode文字プロパティで「数字」を表す
- **安全性重視**: 制御文字や危険なUnicode文字を除外
- **パフォーマンス配慮**: 完全Unicode対応よりも高速

### 正規表現の詳細解説

#### ステップ宣言: `^---\s*([\p{L}\p{N}_\-\s]+?)\s*---$`

- `^`: 行の開始
- `---`: リテラル文字列
- `\s*`: 0回以上の空白文字
- `([\p{L}\p{N}_\-\s]+?)`: ステップ名（文字・数字・アンダーバー・ハイフン・空白）
- `$`: 行の終了

#### 単行引数: `^([\p{L}\p{N}_\-]+)\[([\p{L}\p{N}_]+):((?:[^<\]]|\\.)*?)\]$`

- `([\p{L}\p{N}_\-]+)`: ステップ名（文字・数字・アンダーバー・ハイフン）
- `\[`: リテラル`[`
- `([\p{L}\p{N}_]+)`: 引数名（文字・数字・アンダーバー）
- `:`: 区切り文字
- `((?:[^<\]]|\\.)*?)`: 値（エスケープ処理対応、`<`や`]`で終了しない限り続く）
- `\]$`: リテラル`]`と行終了

#### 複数行開始: `^([\p{L}\p{N}_\-]+)\[([\p{L}\p{N}_]+):<<<(.*)$`

- `([\p{L}\p{N}_\-]+)`: ステップ名（段階的Unicode対応）
- `([\p{L}\p{N}_]+)`: 引数名（段階的Unicode対応）
- `<<<`: ヒアドキュメント開始マーカー
- `(.*)`: 終端に空白文字のみ許可（警告対象）
...
4. **Unicode正規化**: セキュリティのため、内部的にNFC正規化を推奨
...
```typescript
// サポートする文字
const STEP_NAME_PATTERN = /^[\p{L}\p{N}_\-\s]+$/u; // 文字・数字・記号・空白
const ARG_NAME_PATTERN = /^[\p{L}\p{N}_]+$/u; // 文字・数字・アンダーバーのみ
...
| 機能              | 概要                                     |
| ----------------- | ---------------------------------------- |
| 完全Unicode対応   | \p{Script}による詳細な文字体系サポート   |
